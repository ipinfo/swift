...
final class ipinfoKitTests: XCTestCase {
    ...
    func testBogonIPv4() {
        let expectation = expectation(description: "Bogon IP request")
    ...
    func testGetBash() {
        let expectation = expectation(description: "IP details request")
        IPINFO.shared.getBatch(
            ipAddresses: ["AS123", "8.8.8.8", "9.9.9.9/hostname", "2001:0:c000:200::0:255:1", "0.0.0.0"],
            withFilter: false) { status, response, _ in
                switch status {
                case .success:
                    guard let response else { return }
