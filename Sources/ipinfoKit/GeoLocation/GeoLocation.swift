        ...
        if isBogonIP(ip) && ip.isValidIP == .IPv4{
            ...
            if CachingManager.shared.getDataFromCache()[ip] == nil { // Check if the IP data is present in the cache
                Service.shared.requestAPI(URL: .geoLocation(ipAddress: ip), method: .get) { status, data, msg in
                    ...
                    case .failure:
                        completion(.failure, nil, msg) // Call the completion handler with failure status and empty data, indicating that API request failed
                    }
                }
            } else {
                completion(.success, CachingManager.shared.getDataFromCache()[ip], "Success") // If IP data is present in cache, call the completion handler with success status and the cached data
            }
        }
    }
}
